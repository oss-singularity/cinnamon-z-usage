...
def verify_composition(actor, panel, crop, screen, variant):
    ...
    if (
        aw <= 0
        or ah <= 0
        or not (x <= ax and y <= ay and ax + aw <= x + width and ay + ah <= y + height)
    ):
    ...
    if False:
        ...
        if (
            panel[0] - actor[0] - actor[2] != 48
            or screen[1] - actor[1] - actor[3] != 48
        ):
...
def main():
    ...
    if (
        args.extension.name != "transparent-panels@germanfr"
        or not args.extension.is_dir()
    ):
        parser.error(
            "Public captures require the installed transparent-panels@germanfr extension"
        )
    if not args.extension_config.is_file():
        parser.error(
            "Public captures require the reference panel transparency settings"
        )
    ...
    subprocess.run(
        [str(ROOT / "install.sh")],
        check=True,
        env={**os.environ, "XDG_DATA_HOME": str(stage)},
    )
    ...
    source_paths += [
        str(path.relative_to(ROOT)) for path in sorted(UI.iterdir()) if path.is_file()
    ]
    source_paths += ["icon.png"] + [
        str(path.relative_to(ROOT))
        for path in sorted((ROOT / "icons").iterdir())
    ...
    for name, variant, mode, capture_env, panel_crop in SPECS:
        ...
        alpha_match = re.search(
            r"private-panel-alpha=(\d+)", (output / f"{name}.log").read_text()
        )
        ...
        actor_geometry = [
            int(value) for value in geometry.read_text().strip().split(",")
        ]
        ...
        verify_composition(
            actor_geometry, panel_geometry, crop_geometry, screen_geometry, variant
        )
        ...
        if False and min(corner_rgb) < 60:
            raise ValueError(
                "Modal backdrop is too dark for the approved blue composition"
            )
        is_popup = variant in {
            "overview",
            "basic",
        ...
        if is_popup and actor_geometry[2] != 419:
            raise RuntimeError(
                f"{name}: expected 419 px popup actor plus 1 px edge, got {actor_geometry[2]}"
            )
        manifest[name + ".png"] = {
            "surface": (
                "popup"
            ...
            "dimensions": subprocess.check_output(
                ["identify", "-format", "%wx%h", str(image)], text=True
            ),
        }
