...
const AppletClass = new Function(
  "imports",
  "require",
...
)(
  {
    ...
    ui: { applet: { Applet: class {}, AppletPopupMenu: class {} } },
    misc: {},
    gi: { St: { Side: { RIGHT: 1 } } },
  },
  () => ({}),
...
// The grid is statically centered: the popup width is locked and the grid
// width is fixed, so the BinLayout placement alone holds the design offset.
// The former dynamic centering read mid-relayout geometry and wrote a
// translation that doubled with the BinLayout centering - the bistable
// +33px button jump. The sync must therefore never leave a translation on
// the frame, whatever stale geometry a pass reads.
    ...
    for (const error of [-0.0001220703125, 0, 0.0001220703125]) {
      ...
      applet.menu = {
        ...
        actor: {
          ...
          get_transformed_position() {
            return [menuX - error, 0];
          },
          get_transformed_size() {
            return [menuWidth - error, 665];
          },
        },
      };
      ...
      for (let refresh = 0; refresh < 3; refresh++) {
        ...
        if (frame.translation_x !== 0) {
          throw new Error(
            `Action grid translation survived: width=${menuWidth}, ` +
...
print(
  "Content alignment: rings keep their designed positions, the plot edge follows the button grid.",
);
...
// designed offset instead of staying vanished.
{
  ...
  const ring = {
    ...
    get_transformed_position() {
      return [menuX + 348 + this.translation_x, 0];
    },
    get_transformed_size() {
      return [52, 52];
    },
  ...
  if (ring.translation_x !== 1) {
    throw new Error(
      `Settle alignment failed: tx=${ring.translation_x}, expected=1`,
    );
  }
  ...
  if (flung.translation_x !== 0) {
    throw new Error(
      `Fling reset failed: tx=${flung.translation_x}, expected=0 (home)`,
    );
  }
  applet._syncContentRightEdges();
  if (flung.translation_x !== 1) {
    throw new Error(
      `Fling recovery failed: tx=${flung.translation_x}, expected=1`,
    );
  }
  ...
  if (header.translation_x !== -13) {
    throw new Error(
      `Header home reset failed: tx=${header.translation_x}, expected=-13`,
    );
  }
  applet._syncContentRightEdges();
  if (header.translation_x !== 1) {
    throw new Error(
      `Header recovery failed: tx=${header.translation_x}, expected=1`,
    );
  }
}
print(
  "Refresh guard: stale anchor passes are rejected, flung rings self-heal.",
);
...
// agreeing passes (the one-poll refresh flash of the green rings).
{
  ...
  const ring = {
    ...
    bias: 0,
    get_transformed_position() {
      return [menuX + 348 + this.translation_x + this.bias, 0];
    },
    get_transformed_size() {
      return [52, 52];
    },
  ...
  if (ring.translation_x !== -299) {
    throw new Error(
      `repeated honest delta not applied: tx=${ring.translation_x}`,
    );
  }
  ...
  if (ring.translation_x !== -299) {
    throw new Error(
      `first correction pass applied early: tx=${ring.translation_x}`,
    );
  }
  applet._syncContentRightEdges();
  if (ring.translation_x !== 1) {
...
// must toggle without rebuilding.
{
  ...
  if (rebuilds !== 0 || toggles !== 1 || applet.menu.isOpen !== false) {
    throw new Error(
      `closing click rebuilt: rebuilds=${rebuilds}, toggles=${toggles}`,
    );
  }
  applet.on_applet_clicked();
  if (rebuilds !== 1 || toggles !== 2 || applet.menu.isOpen !== true) {
    throw new Error(
      `opening click did not rebuild: rebuilds=${rebuilds}, toggles=${toggles}`,
    );
  }
}
