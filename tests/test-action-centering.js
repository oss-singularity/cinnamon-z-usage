...
// The "manchmal" flush drift: the painted consumed end wobbles a few
// pixels with the digit glyphs while the anchor stays put. The sync pins
// the suffix group rigidly onto the grid edge on every pass.
{
    const menuX = 1461;
    const makeLabel = x => ({
        x,
        ...
        get_transformed_size() { return [50, 16]; }
    });
    ...
    // The painted end wobbled 12px short (hinting on new digits): the pin
    // moves the whole group rigidly so the end lands exactly on the edge.
    expiry.x = menuX + 318;
    applet._syncContentRightEdges();
    const end = expiry.get_transformed_position()[0] + 50;
    if (end !== right) {
        throw new Error(`suffix end not pinned: end=${end}, expected=${right}`);
    }
    const deltas = [separator, expires, expiry].map(label => label.translation_x);
    if (new Set(deltas).size !== 1) {
        throw new Error(`suffix group did not move rigidly: ${deltas}`);
    }

    // The next wobble in the other direction is compensated identically.
    expiry.x = menuX + 330;
    applet._syncContentRightEdges();
    if (expiry.get_transformed_position()[0] + 50 !== right) {
        throw new Error("suffix pin is not idempotent across wobble directions");
    }
}
print("Credits suffix pin: the painted end always finishes on the grid edge.");
