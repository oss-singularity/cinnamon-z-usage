...
referenced = set(
    re.findall(r"docs/model-limits/([a-z0-9-]+\.png)", (ROOT / "README.md").read_text())
)
if referenced != set(manifest):
    raise SystemExit(
        f"Screenshot inventory mismatch: {referenced.symmetric_difference(manifest)}"
    )
for name, record in manifest.items():
    try:
        ...
        if (
            record["extension"] != "transparent-panels@germanfr"
            or not 0 <= record["panelBackgroundAlpha"] < 255
        ):
        ...
        raise SystemExit(
            f"Screenshot composition is not verified: {name}: {error}"
        ) from error
    if name.startswith("usage-menu") or name == "bucket-tooltip.png":
        if (
            record.get("surface") != "popup"
            or record.get("actorGeometry", [0, 0, 0])[2] != 419
        ):
            raise SystemExit(
                f"Screenshot has no verified 420 px popup width (419 px actor + edge): {name}"
            )
    ...
    if record.get("sourcePolicy") == "historical":
        if name not in {"topbar.png", "vertical-panel.png"}:
            raise SystemExit(
                f"Historical source policy is limited to panel anchors: {name}"
            )
        if not re.fullmatch(r"\d+\.\d+\.\d+", str(record.get("sourceRelease", ""))):
            raise SystemExit(
                f"Historical screenshot is missing its source release: {name}"
            )
        continue
...
print(
    f"UI inventory: {len(manifest)} images match their producing source and output hashes."
)
